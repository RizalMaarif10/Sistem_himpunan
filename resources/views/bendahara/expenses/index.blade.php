...
<div class="mb-6 flex items-center justify-between">
  <h1 class="text-2xl md:text-3xl font-bold">Pengeluaran</h1>
  <a href="{{ route('bendahara.expenses.create') }}" class="px-4 py-2 rounded-lg bg-slate-900 text-white hover:bg-blue-600 text-sm">+ Tambah</a>
</div>
    ...
    <tbody>
      ...
      <tr class="border-t">
        ...
        <td class="px-4 py-2 text-right">
          <a href="{{ route('bendahara.expenses.edit',$t) }}" class="px-3 py-1.5 rounded-lg border text-sm">Edit</a>
          <form method="POST" action="{{ route('bendahara.expenses.destroy',$t) }}" class="inline" onsubmit="return confirm('Hapus data?')">
            @csrf @method('DELETE')
